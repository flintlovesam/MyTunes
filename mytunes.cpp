...
MyTunes::MyTunes():view(this)
{
	...
	string dbTitle = "beatles.db";
	char *c_dbTitle =  const_cast<char*>(dbTitle.c_str());
	db = new Database(c_dbTitle);
}
...
void MyTunes::executeCMDFOLLOW(Command cmd){
	//  follow -u userid -p playlist_name -f userid
	//  follow -u userid -p playlist_name -f stop

	enum arguments {FOLLOW, _U, USERID, _P, PLAYLIST_NAME, _F, FLAG};
	if(cmd.isTokenAt(FLAG, "stop")){
		executeFollowStop(cmd);
	}
	else{
		executeFollowUser(cmd);
	}
}

void MyTunes::executeFollowUser(Command cmd){

	view.printOutput("EXECUTING: FOLLOW USER " + cmd.getCommandString());

	string user_id = cmd.getToken(2);
	string user_id_2 = cmd.getToken(6);
	if(user_id == user_id_2) {view.printOutput("cannot overwrite own user."); return;}
	string setlistName = cmd.getToken(4);

	User* userOne = users.findByID(user_id);
	if(userOne == NULL) {view.printOutput("user one userID does not exist."); return;}
	Playlist * playlistOne = userOne->findPlaylist(setlistName);
	if(playlistOne == NULL) {view.printOutput("user one playlist does not exist."); return;}

	User* userTwo = users.findByID(user_id_2);
	if(userTwo == NULL) {view.printOutput("user two userID does not exist."); return;}
	Playlist * playlistTwo = userTwo->findPlaylist(setlistName);
	if(playlistTwo == NULL) {view.printOutput("user two playlist does not exist."); return;}
	if(user_id == user_id_2) {view.printOutput("cannot overwrite own user."); return;}

	//for (int i = 0; i < playlistTwo->getTracks().size(); i++){
	//	playlistOne->addTrack(* playlistTwo->getTracks()[i]);
	//}
	playlistOne->startFollowing(*playlistTwo);


}

void MyTunes::executeFollowStop(Command cmd){
...
void MyTunes::executeCMDSHOW(Command cmd){
	...
	if(cmd.isTokenAt(COLLECTION, "songs") && !cmd.hasToken("-s")){
		...
		for(vector<vector<string> >::iterator it = result.begin(); it < result.end(); ++it)
		{
			...
			view.printOutput("Song ID=" + row.at(0) + ", Title=" + row.at(1) + " ,Composer=" + row.at(2));
	}
...
void MyTunes::executeAddSong(Command cmd){
	...
	db->query((string("INSERT INTO songs VALUES(") + cmd.getToken(ID) + string(", '") + cmd.getToken(TITLE) + string("', '") + cmd.getToken(COMPOSER) + string("');")).c_str());
	view.printOutput("EXECUTING: ADD SONG IN DATABASE /n");
}
...
void MyTunes::executeAddRecording(Command cmd){
	...
	db->query((string("INSERT INTO recordings VALUES(") + cmd.getToken(ID) + string(", '") + cmd.getToken(TITLE) + string("', '") + cmd.getToken(ARTIST) + string("', '") + cmd.getToken(PRODUCER) + string("', '") + cmd.getToken(YEAR) + string("');")).c_str());
	view.printOutput("EXECUTING: ADD RECORDINGS IN DATABASE /n");
}
void MyTunes::executeAddTrack(Command cmd){
	...
	db->query((string("INSERT INTO tracks VALUES(") + cmd.getToken(RECORDING_ID) + string(", '") + cmd.getToken(SONG_ID) + string("', '") + cmd.getToken(TRACK_NUMBER) + string("');")).c_str());
	view.printOutput("EXECUTING: ADD TRACK IN DATABASE /n");
}
void MyTunes::executeAddUser(Command cmd){
	...
	db->query((string("INSERT INTO users VALUES(") + cmd.getToken(USERID) + string(", '") + cmd.getToken(NAME) + string("');")).c_str());
	view.printOutput("EXECUTING: ADD USERS IN DATABASE /n");
}
void MyTunes::executeAddPlaylist(Command cmd){
	...
	db->query((string("INSERT INTO playlists VALUES(") + cmd.getToken(USERID) + string(", '") + cmd.getToken(PLAYLIST_NAME) + string("');")).c_str());
	view.printOutput("EXECUTING: ADD PLAYLIST IN DATABASE /n");
}
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
	...
	//playlist->notify();
	db->query((string("INSERT INTO playlist_track VALUES(") + cmd.getToken(USERID) + string(", '") + cmd.getToken(PLAYLIST_NAME) + string(", '") + cmd.getToken(TRACK_ID)+ string("');")).c_str());
	view.printOutput("EXECUTING: ADD PLAYLIST TRACK IN DATABASE /n");
...
void MyTunes::executeDeleteRecording(Command cmd){
	...
	db->query((string("DELETE FROM songs WHERE id=") + cmd.getToken(ID) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE RECORDING IN DATABASE /n");
...
void MyTunes::executeDeleteUser(Command cmd){
	...
	db->query((string("DELETE FROM users WHERE user_id=") + cmd.getToken(USER_ID) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE USER IN DATABASE /n");
}
void MyTunes::executeDeleteUserPlaylist(Command cmd){
	...
	db->query((string("DELETE FROM playlists WHERE user_id=") + cmd.getToken(user_id) + string("AND playlist_name=") + cmd.getToken(playlistName) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE PLAYLIST IN DATABASE /n");
}
void MyTunes::executeDeleteUserPlaylistTrack(Command cmd){
	...
	db->query((string("DELETE FROM playlist_tracks WHERE user_id=") + cmd.getToken(user_id) + string("AND playlist_name=") + cmd.getToken(playlistName) + + string("AND song_id=") + cmd.getToken(trackIDstring) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE PLAYLIST TRACK IN DATABASE /n");
}
void MyTunes::executeDeleteTrack(Command cmd){
	...
	tracks.remove(*track);
	db->query((string("DELETE FROM tracks WHERE track_number=") + cmd.getToken(trackIDstring) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE TRACK IN DATABASE /n");
}
...
void MyTunes::executeDeleteSong(Command cmd){
	...
	db->query((string("DELETE FROM songs WHERE sid=") + cmd.getToken(ID) + string(";")).c_str());
	view.printOutput("EXECUTING: DELETE SONG IN DATABASE /n");
}
