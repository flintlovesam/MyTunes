...
cout << this << "Playlist update" << endl;
if(tracks == following->tracks) return;
if(following) tracks = following->tracks;
notify();
}
void Playlist::stopFollowing(){
	cout << this << "Playlist stop following" << endl;
	if(following) following->dettach(*this);
	//notify();

}
void Playlist::startFollowing(Playlist & followPlaylist){
	cout << this << "Playlist start following" << endl;
	//stopFollowing();
	following = &followPlaylist;
	if(following) following->attach(*this);
	//notify();
}
